...
#         self.S('li.chapterItem:nth-child(1)').click()
#
...
#         # e_html = html.encode('utf-8')
#         html_list = html.split(' ')
#         with open(save_path, 'w') as f:  # encoding='utf-8'
#             for h in html_list:
#                 try:
#                     f.write(h+' ')
...
class WeRead:
    ...
    def __init__(self, driver: WebDriver, patience=15, debug=False):
        ...
        self.patience = patience
        self.path = os.path.dirname(os.path.realpath(__file__))
        self.big_book_flag = False
        self.big_book_sort = 0
    ...
    def switch_to_context(self):
        ...
        try:
            self.S('button.catalog').click()
            self.S('li.chapterItem:nth-child(200)').click()
            raise BigBookError("book页数太大")
        except BigBookError as e:
            raise BigBookError("book页数太大")
        except Exception as e:
            # 如果正常报错，证明书籍无200章，则继续正常获取
            # self.S('button.catalog').click()
            self.S('li.chapterItem:nth-child(1)').click()

        # try:
        # self.S('button.catalog').click()
        # self.S('li.chapterItem:nth-child({})'.format(self.big_book_sort*200+1)).click()
        # except:
        #     self.big_book_flag = False
        #     self.big_book_sort = 0
        #     self.continue_flag = False
    ...
    def get_html(self, book_url):
        ...
        while True:
            ...
            try:
                ...
                # else:
                #     time.sleep(1)
                self.driver.find_element(by=By.CSS_SELECTOR, value=".app_content")
                readerFooter = self.S('.readerFooter_button,.readerFooter_ending')
            except Exception:
                break
            readerFooterClass = readerFooter.get_attribute('class')
    ...
    def scan2html(self, book_url, save_at='.', show_output=True, book_name_add=""):
        html = self.get_html(book_url)
        # 超过200页的图书进行分本操作
        if self.big_book_flag:
            each_book_name = self.current_book_name + '_' + str(self.big_book_sort)
            self.download_html(html, save_at=save_at, book_name=each_book_name+'_'+book_name_add,
                               show_output=show_output)
        else:
            self.download_html(html, save_at=save_at, book_name=self.current_book_name+'_'+book_name_add,
                               show_output=show_output)
